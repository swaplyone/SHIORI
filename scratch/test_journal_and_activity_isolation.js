...
const JWT_SECRET = process.env.JWT_SECRET || 'shiori-secret-key-change-in-production';
const PORT = 3000;

function createToken(userId, email, username) {
  return jwt.sign({ id: userId, email, username }, JWT_SECRET, { expiresIn: '1h' });
}
...
async function runTests() {
  ...
  const { queryAll, queryOne, runQuery } = await import('../server/dist/db/index.js');
  const { taskReportService } = await import('../server/dist/services/taskReport.service.js');
  const { extractDayString, normalizeDate } = await import('../server/dist/utils/dateRange.js');
  ...
  const wsAId = uuidv4();
  await runQuery(`INSERT INTO workspaces (id, name, slug, owner_id) VALUES (?, 'WS A', 'ws-a', ?)`, [wsAId, userAId]);
  await runQuery(`INSERT INTO workspace_members (id, workspace_id, user_id, role) VALUES (?, ?, ?, 'owner')`, [uuidv4(), wsAId, userAId]);
  await runQuery(`INSERT INTO projects (id, workspace_id, name, slug, github_repo_name, created_by) VALUES (?, ?, 'SHIORI Core', 'shiori-core', 'SHIORI', ?)`, [projAId, wsAId, userAId]);
  ...
  const wsBId = uuidv4();
  await runQuery(`INSERT INTO workspaces (id, name, slug, owner_id) VALUES (?, 'WS B', 'ws-b', ?)`, [wsBId, userBId]);
  await runQuery(`INSERT INTO workspace_members (id, workspace_id, user_id, role) VALUES (?, ?, ?, 'owner')`, [uuidv4(), wsBId, userBId]);
  await runQuery(`INSERT INTO projects (id, workspace_id, name, slug, github_repo_name, created_by) VALUES (?, ?, 'Compiler SO', 'compiler-so', 'compiler-SO', ?)`, [projBId, wsBId, userBId]);
  ...
  const journalTodayB = await request({
    hostname: 'localhost',
    port: PORT,
  ...
  const journalWeeklyB = await request({
    hostname: 'localhost',
    port: PORT,
  ...
  await runQuery(`INSERT INTO projects (id, workspace_id, name, slug, github_repo_name, created_by) VALUES (?, ?, 'Team Shared Alpha', 'team-shared-alpha', 'team-alpha', ?)`, [sharedProjId, wsAId, userAId]);
  ...
  const activityBShared = await request({
    hostname: 'localhost',
    port: PORT,
    path: '/api/activity',
    method: 'GET',
  ...
  const privateTaskRes = await request({
    hostname: 'localhost',
    port: PORT,
