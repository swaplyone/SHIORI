  ...
  return (
    <div className="space-y-8 select-none font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((proj) => {
          const total = proj.total_tasks || 12;
          const completed = proj.completed_tasks || 8;
          const inProg = proj.in_progress_tasks || 2;
          const review = proj.review_tasks || 1;
          const failedCI = proj.failed_ci_tasks || (proj.slug.includes('compiler') ? 1 : 0);
                ...
                <div className="space-y-2 pt-2 border-t border-eink-border/60 text-xs font-technical">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-eink-text">
                      {Math.round((completed / (total || 1)) * 100)}% complete
                    </span>
                  </div>
                  ...
                  <div className="w-full bg-eink-bg h-2 border border-eink-border rounded overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.round((completed / (total || 1)) * 100)}%` }}
                    />
                  </div>
